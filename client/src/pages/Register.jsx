...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!passwordRegex.test(password)) {
      toast.error('Password must be 8+ chars, with an uppercase, lowercase, number, and special character.', { duration: 4000 });
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        if (res.message && res.message.includes('email-already-in-use')) {
          toast.loading('Account exists. Attempting login...', { id: 'login-toast' });
          const loginRes = await login(email, password);
          if (loginRes.success) {
            toast.success('Logged in successfully!', { id: 'login-toast' });
            navigate('/products');
          } else {
            toast.error(loginRes.message || 'Email already registered. Incorrect password.', { id: 'login-toast', duration: 5000 });
          }
        } else {
      ...
      <div style={{ width: '100%', maxWidth: '400px' }}>
        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #EBEBEB', padding: '36px 32px', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
          <div style={{ textAlign: 'center', marginBottom: '28px' }}>
            ...
            <h1 style={{ fontSize: '20px', fontWeight: 800, color: '#111827', marginBottom: '4px' }}>Create account</h1>
            <p style={{ fontSize: '13px', color: '#9CA3AF' }}>Join thousands of happy customers</p>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', margin: '20px 0', color: '#9CA3AF', fontSize: '13px' }}>
            ...
            <span style={{ padding: '0 10px' }}>or sign up with email</span>
            <hr style={{ flex: 1, border: 'none', borderTop: '1px solid #E5E7EB' }} />
          </div>

          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#374151', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>Full Name</label>
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="John Doe" className="input-base" style={{ fontSize: '14px' }} />
            </div>
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" className="input-base" style={{ fontSize: '14px' }} />
            </div>
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Min 8 chars, A-Z, a-z, 0-9, symbol" className="input-base" style={{ fontSize: '14px', paddingRight: '40px' }} />
          ...
          <p style={{ textAlign: 'center', marginTop: '18px', fontSize: '13px', color: '#6B7280' }}>
            ...
            <Link to="/login" style={{ color: '#22C55E', fontWeight: 700, textDecoration: 'none' }}>Sign In</Link>
          </p>
        </div>
      </div>
    </div>
